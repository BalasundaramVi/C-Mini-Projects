...
void Player::move(int dir)
{
    ...
    if (dir == 1)
    {
        if (m_row == m_pit->rows())
            return;
        else if (m_row <= m_pit->rows()-2 && m_pit->numberOfScorpionsAt(m_row+1, m_col)==0)
            m_row++;
        else if (m_row <= m_pit->rows()-2 && m_pit->numberOfScorpionsAt(m_row+1, m_col)>=1)
        {
            if (m_row <= m_pit->rows()-3 && m_pit->numberOfScorpionsAt(m_row+2, m_col)==0)
            {
                m_row = m_row + 2;
                m_pit->destroyOneScorpion(m_row+1, m_col);
            }
            else if (m_row <= m_pit->rows()-3 && m_pit->numberOfScorpionsAt(m_row+2, m_col)>=1)
            {
                m_row = m_row + 2;
                m_pit->destroyOneScorpion(m_row+1, m_col);
                setDead();
    ...
    if (dir == 3)
    {
        if (m_col == m_pit->cols())
            return;
        else if (m_col <= m_pit->cols()-2 && m_pit->numberOfScorpionsAt(m_row, m_col+1)==0)
            m_col++;
        else if (m_col <= m_pit->cols()-2 && m_pit->numberOfScorpionsAt(m_row, m_col+1)>=1)
        {
            if (m_col <= m_pit->cols()-3 && m_pit->numberOfScorpionsAt(m_row, m_col+2)==0)
            {
                m_col = m_col + 2;
                m_pit->destroyOneScorpion(m_row, m_col+1);
            }
            else if (m_col <= m_pit->cols()-3 && m_pit->numberOfScorpionsAt(m_row, m_col+2)>=1)
            {
                m_col = m_col + 2;
                m_pit->destroyOneScorpion(m_row, m_col+1);
                setDead();
    ...
    for (r = 0; r < rows(); r++)
        for (c = 0; c < cols(); c++)
        {
            if (numberOfScorpionsAt(r, c) == 1)
                grid[r][c] = 'S';
            else if (numberOfScorpionsAt(r, c) >= 2 && numberOfScorpionsAt(r, c)<=8)
                grid[r][c] = char(numberOfScorpionsAt(r, c)+48);
